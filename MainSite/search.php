...
<p>Search:  <input class="artisttext" type="text" name="keyword"> &nbsp;
	...
	if (isset($_POST["keyword"]) && strlen ($_POST["keyword"]) > 2)
	{
		...
		if (mysql_num_rows($result) > 0) {
			...
			while($row = mysql_fetch_assoc($result)) {
				if (strpos($row["artist"], $search_keyword) !== false ||
					strpos($row["song"], $search_keyword) !== false ||
					strpos($row["category"], $search_keyword) !== false)
					{
